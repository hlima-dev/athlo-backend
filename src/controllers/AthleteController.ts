...
import { AmpLevel, AthleteStatus, Sport, UserRole, UserStatus } from '@prisma/client'
...
const createAthleteSchema = z.object({
  ...
  // Senha não tem mais default fraca — será gerada aleatoriamente se não enviada
...
export class AthleteController {
  async create(req: Request, res: Response): Promise<void> {
    ...
    // Gera senha temporária aleatória se não fornecida
    const rawPassword = data.password ?? crypto.randomBytes(12).toString('base64url')
    const hashedPassword = await bcrypt.hash(rawPassword, env.BCRYPT_SALT_ROUNDS)
    ...
    res.status(201).json(successResponse(athlete, 'Atleta cadastrado com sucesso'))
  }
